...
/**
 * In-game staff commands (`!give`). Staff commands are just chat triggers
 * like `!inv`/`!link` — the pack never decides who may run them. It only
 * forwards the actor's own persistentId (captured at join, so it can't be
 * spoofed by the client) and lets the backend authorize via the staff
...
export function tryHandleAdminCommand(message, player, deps) {
  ...
  for (const command of ADMIN_COMMANDS) {
    if (lower === command || lower.startsWith(`${command} `)) {
      const argv = message.trim().split(/\s+/).slice(1);
      handleGive(player, deps, argv); // fire-and-forget: result lands in chat
      return true;
    }
  }
  return false;
